...
with tab2:
    ...
    with col1:
        ...
        town_df = pd.DataFrame(list(town_map.items()), columns=['Town', 'Numeric Code'])

        st.subheader(":violet[Town to Numeric Code Mapping]")
        st.markdown("""
            <p style='font-size: 16px;'>This table maps each town in Singapore to a unique numerical code.
            
            This mapping is often used for converting categorical data (town names) into numerical features for machine learning models.</p>
        """, unsafe_allow_html=True)

        
        st.dataframe(town_df, hide_index=True)
    ...
    with col2:
        ...
        flat_model_df = pd.DataFrame(list(flat_model_map.items()), columns=['Flat Model', 'Numeric Code'])

        st.subheader(":violet[Flat Model to Numeric Code Mapping]")
        st.markdown("""
            <p style='font-size: 16px;'>This table maps different flat models to unique numerical codes. 
            
            This mapping is often used in machine learning models to convert categorical data (flat models) into numerical features.</p>
        """, unsafe_allow_html=True)

     
        st.dataframe(flat_model_df, hide_index =True)
...
with col1:
    ...
    a1 = st.text_input("Town Encoded", placeholder="Please refer to Town to Numeric Code Mapping table in Encoding Details Tab")
